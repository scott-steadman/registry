...
def patch_rails_for_ruby_27
  ...
  as_gem_spec = Gem.loaded_specs.values.find { |spec|
    spec.name == 'activesupport' && (spec.version.to_s.start_with?('3.0.') || spec.version.to_s.start_with?('3.1.') || spec.version.to_s.start_with?('3.2.'))
  }
  if as_gem_spec
    ...
    if as_gem_spec.version.to_s.start_with?('3.2.')
  ...
  # Patch ActiveRecord sqlite3_adapter to allow newer sqlite3 gem versions (Rails 3.1 and 3.2)
  ar_gem_spec = Gem.loaded_specs.values.find { |spec| spec.name == 'activerecord' && (spec.version.to_s.start_with?('3.1.') || spec.version.to_s.start_with?('3.2.')) }
  if ar_gem_spec
    ...
    if File.exist?(sqlite3_adapter_file)
      ...
      # Fix: Remove version constraint for sqlite3 gem (Rails 3.1 uses ~> 1.3.4, Rails 3.2 uses ~> 1.3.5)
      if content.include?("gem 'sqlite3', '~> 1.3.4'")
        content.gsub!(/gem 'sqlite3', '~> 1\.3\.4'/, "gem 'sqlite3'")
        File.write(sqlite3_adapter_file, content)
      elsif content.include?("gem 'sqlite3', '~> 1.3.5'")
        content.gsub!(/gem 'sqlite3', '~> 1\.3\.5'/, "gem 'sqlite3'")
        File.write(sqlite3_adapter_file, content)
      end
    end

    # Patch ActiveRecord HasManyAssociation for Ruby 2.7+ (Rails 3.1 only, Rails 3.2 doesn't have this issue)
    ...
    if ar_gem_spec.version.to_s.start_with?('3.1.') && File.exist?(has_many_file)
